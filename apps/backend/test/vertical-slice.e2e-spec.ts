...
} from '../src/auth/supabase-client.provider';
...
// M15's own established pattern (auth/supabase-auth.integration.spec.ts)
// — skipped, not failed, when no real dev Supabase project is
 ...
 * real, non-production dev Supabase project (Decision J.5), entirely
 * with synthetic/fictional data.
 *
...
describeIfEnabled(
  ...
  () => {
    ...
    const supabaseAuthService = new SupabaseAuthService(parentRepository);
    ...
    beforeAll(async () => {
      if (hasSupabaseCredentials) {
        ...
        // No real dev Supabase project configured in this environment
        // — same fictional-auth-ref placeholder convention every
        // non-Supabase M14-M18 fixture in this repository already
        // uses (e.g. leo-cross-child-isolation.integration.spec.ts).
        supabaseUserId = `fictional-auth-ref-${owner.id}`;
      }
    ...
    // Only runs for real against the dev Supabase project (Decision
    // J.5) when SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY are configured
    // — identical skip condition to M15's own
    // supabase-auth.integration.spec.ts. This is the "parent signup"
    // half of the flow's first step; Parent-record creation itself
    // (above, in beforeAll) is unconditional, per
    // auth/supabase-auth.service.ts's own documented scope ("does not
    // create Parent records — that remains an identity-family
    // concern").
    (hasSupabaseCredentials ? it : it.skip)(
      'step 1 — parent signup: a real dev Supabase access token resolves to the Parent record (M15)',
      async () => {
        const client = getSupabaseClient();
        ...
        const resolvedParent = await supabaseAuthService.resolveParentFromAccessToken(
          data.session.access_token,
        );
        expect(resolvedParent.id).toBe(owner.id);
        expect(resolvedParent.authIdentityRef).toBe(supabaseUserId);
      },
    );
